...
def formating_text_for_bar(Dataset, groupedby, sortedby, nb_see=15):
    ...
    data :          formated data
    """
    data = Dataset.groupby(groupedby).nunique().drop(columns=groupedby).reset_index()
...
def formating_age_for_pie(Dataset, dates1, dates2):
    ...
    dico_age = {'<20': tens['Unnamed: 0'][0],
                '20-30': twenty['level_0'][1],
                '30-40': thirty['level_0'][1],
                '40-50': fourty['level_0'][1],
                '50-60': fifty['level_0'][1],
                '>60': sixtyplus['level_0'][1]}
